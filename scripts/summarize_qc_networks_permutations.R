...
usage: Rscript summarize_qc_networks_permutations.R [folder_base] [...]
...
optional argument:
  ...
  --iters=[VALUE]             number of permutations
...
if (length(args) > 1) {
  ...
  opt_args <- args[-1]
  opt_args_allowed <- c("--meff-model", "--norm-method", "--minsize", "--pamStage", "--iters")
...
for (model in meff_model) {
  for (norm in norm_method) {
    ...
    # write summary
    fwrite(qc_results, file = paste0(folder_base, "/meff_", model, "/norm_", norm,
                            "/permutation/summary_qc_networks_permutations.txt"),
           quote = FALSE, sep = "\t", na = NA, row.names = FALSE)

    output_folder <- paste0(folder_base, "/meff_", model, "/norm_", norm,
                            "/permutation/qc_networks_permutations")
    if (!dir.exists(output_folder)) dir.create(output_folder, recursive = TRUE)

    # number of markers per network
    plot1 <- qc_results %>%
      filter(source == "TOM") %>%
      group_by(meff_model, norm_method, permu_iter, minsize, pamStage) %>%
      summarize(n_markers = n()) %>%
      ungroup() %>%
      group_by(meff_model, norm_method, permu_iter) %>%
      summarize(n_markers = mean(n_markers)) %>%
      ungroup() %>%
      ggplot() +
      geom_col(aes(x = paste(meff_model, norm_method, permu_iter, sep = "_"), y = n_markers)) +
      theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
            panel.grid.minor = element_blank()) +
      labs(x = "network", y = "number of markers")

    ggsave(plot = plot1, filename = paste0(output_folder, "/n_markers.pdf"),
           device = "pdf", width = 8, height = 0.5 * iters, limitsize = FALSE)

    # number of markers per module - with grey module
    plot2 <- qc_results %>%
      filter(source == "TOM") %>%
      group_by(meff_model, norm_method, permu_iter, minsize, pamStage, source, module) %>%
      summarize(n_markers = n()) %>%
      ungroup() %>%
      ggplot() +
      facet_grid2(meff_model + norm_method + permu_iter ~ minsize + pamStage, scales = "free_y", independent = "y") +
    ...
    ggsave(plot = plot2, filename = paste0(output_folder, "/dist_markers.pdf"),
           device = "pdf", width = 12, height = 0.5 * iters, limitsize = FALSE)
    ...
    plot4 <- ggplot(qc_results, aes(x = source, y = kDiff, fill = source)) +
      facet_grid2(meff_model + norm_method + permu_iter ~ minsize + pamStage) +
      geom_violin(draw_quantiles = 0.5, show.legend = FALSE)

    ggsave(plot = plot4, filename = paste0(output_folder, "/dist_kDiff.pdf"),
           device = "pdf", width = 10, height = 0.5 * iters, limitsize = FALSE)
    ...
    plot8 <- qc_results %>%
      ...
      summarize(n_markers = n(),
                median = median(kDiff)) %>%
      ungroup() %>%
      group_by(meff_model, norm_method, permu_iter, minsize, pamStage, source) %>%
      summarize(n_mods = n(),
    ...
    plot10 <- qc_results %>%
      ...
      ungroup() %>%
      ggplot() +
      facet_grid2(meff_model + norm_method + permu_iter ~ minsize + pamStage, scales = "free_x", independent = "x") +
    ...
    ggsave(plot = plot15, filename = paste0(output_folder, "/kDiff_per_network.TOM.pdf"),
           device = "pdf", width = 28, height = 1 * iters, limitsize = FALSE)
    ...
    plot19 <- qc_results %>%
      filter(source == "adjacency") %>%
      group_by(meff_model, norm_method, permu_iter, minsize, pamStage, module) %>%
      summarize(n_markers = n()) %>%
      ungroup() %>%
      group_by(meff_model, norm_method, permu_iter, minsize, pamStage) %>%
      mutate(n_modules = n(),
      ...
      ggplot() +
      facet_nested(metric ~ meff_model + norm_method + permu_iter + minsize + pamStage, scales = "free",
    ...
    ggsave(plot = plot19, filename = paste0(output_folder, "/n_modules_per_network.pdf"),
           device = "pdf", width = 10, height = 0.5 * iters, limitsize = FALSE)
    ...
    plot20 <- qc_results %>%
      ...
      theme(axis.text.x = element_blank(),
            ...
            panel.grid.major.x = element_blank(),
            panel.spacing = unit(0, "lines"))
    ggsave(plot = plot20, filename = paste0(output_folder, "/summary_metrics_per_network.pdf"),
           device = "pdf", width = 10, height = 0.5 * iters, limitsize = FALSE)
    # need to add white spaces between strip groups
    ...
    plot21 <- qc_results %>%
      ...
      theme(axis.text.x = element_blank(),
            ...
            panel.grid.major.x = element_blank(),
            panel.spacing = unit(0, "lines"))
    ggsave(plot = plot21, filename = paste0(output_folder, "/proportion_positive_kDiff_per_module.pdf"),
           device = "pdf", width = 10, height = 0.5 * iters, limitsize = FALSE)
    # need to add white spaces between strip groups
...
# folder_base <- "analysis/networks/YLD/meff_gwas/norm_minmax/permutation"
