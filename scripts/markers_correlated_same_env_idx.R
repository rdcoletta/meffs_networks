...
usage <- function() {
  cat("
description: check markers in modules that are correlated to the same env idx while
             also considering the markers in LD to these markers.
...
for (idx in mixedsort(unique(mod_env_idx_cor$env_idx))) {
  
  # get only modules associated with that env_idx
  sig_env_idx <- subset(mod_env_idx_cor, env_idx == idx)
  
  # create empy dataframe to store markers from different networks
