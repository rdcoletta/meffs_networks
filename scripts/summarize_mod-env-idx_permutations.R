...
optional argument:
  ...
  --iters=[VALUE]             number of iterations for permutation
...
if (length(args) > 1) {
  ...
  opt_args <- args[-1]
  opt_args_allowed <- c("--minsize", "--pamStage", "--idx-type", "--iters")
...
for (type in idx_type) {
  ...
  for (iter in 1:iters) {
    for (size in minsize) {
      for (pam in pamStage) {
        ...
        network_mod_env <- paste0(folder_base, "/iter", iter,
        ...
        if (class(network_mod_env) != "try-error") {
          ...
          network_mod_env <- data.frame(iter = iter, minsize = size, pamStage = pam,
  ...
  # write summary
  fwrite(mod_env_idx_results, file = paste0(folder_base, "/permutation_module-env-idx_per_network.", type, ".txt"))
  ...
  plot_mod_env_idx <- plot_mod_env_idx +
    facet_grid2(iter ~ minsize + pamStage) +
  ...
  # save plot
  ggsave(filename = paste0(folder_base, "/permutation_mod-env-idx_correlation.", type, ".pdf"),
         plot = plot_mod_env_idx, device = "pdf", width = 4, height = 40)

  # save only most significant correlations
  mod_env_idx_results_sig <- filter(mod_env_idx_results, pval < 0.2)
  fwrite(mod_env_idx_results_sig, file = paste0(folder_base, "/permutation_mod-env-idx_most-sig-corr.", type, ".txt"))
